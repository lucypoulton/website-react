...
export function PronounsCloudPage() {
    ...
    return <>
        ...
        <h4>For servers running ProNouns 3.0.0 and above</h4>
        <div className="pnc-fields">
            ...
            <label htmlFor="plural">Plural <input type="checkbox" id="plural" name="plural" defaultChecked
                                                  onInput={() => dispatch({plural: !state.plural})}/>
                <div>{capitalise(state.subjective || "They")} <b>{state.plural ? "are" : "is"}</b> using
                    a {state.plural ? "plural" : "singular"} pronoun set.
                </div>
            </label>
            {fields.some(field => state[field.property] === '') ?
                <h4>Make sure to fill out all the boxes.</h4> :
                <code>/pronouns
                    set {state.subjective}/{state.objective}/{state.possessiveAdj}/{state.possessivePro}/{state.reflexive}{state.plural ? ":p" : ""}</code>
            }</div>
    </>;
}
